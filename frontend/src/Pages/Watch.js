...
export default function Watch() {
  ...
  const fetchVideo = async () => {
    try {
      ...
      // ✅ FIX: Fetch ad after video data is loaded
      if (res.data._id) {
        await fetchAd(res.data._id);
      }
    } catch (error) {
      console.error("Error fetching video:", error);
    }
  ...
  const fetchAd = async (videoId) => {
    ...
    try {
      console.log("🎯 Fetching ad for video:", videoId);
      const res = await axios.get(`http://localhost:5000/api/ads/${videoId}`);
      
      console.log("🟡 AD RESPONSE:", res.data);
  
      if (res.data && res.data.videoFile) {
        setAd(res.data);
        setShowAd(true);
        setAdTime(0);
        console.log("✅ Ad loaded successfully:", res.data.title);
      } else {
        console.log("⚠️ No ad available for this video");
      }
    } catch (err) {
      console.log("❌ Ad fetch error:", err.message);
      // If no ad found, just continue without showing ad
      setShowAd(false);
    }
  };
          ...
          <div style={{ 
            ...
            <div ref={playerContainerRef} style={styles.videoContainer}>
              <video
                ...
                onEnded={() => {
                  if (showAd) {
                    console.log("✅ Ad ended, showing main video");
                    setShowAd(false);
                    setAdTime(0);
                  } else {
                    handleVideoEnd();
              ...
              {showAd && ad && (
                <div style={{
                  ...
                  📢 Advertisement {adTime >= ad.skipAfter ? `(Skippable)` : `(${Math.max(0, Math.ceil(ad.skipAfter - adTime))}s)`}
                </div>
              )}
