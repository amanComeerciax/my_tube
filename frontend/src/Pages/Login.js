...
import { auth, googleProvider, signInWithPopup } from "../firebase"; // Import your config
  ...
  return (
    <div className="login-container">
      ...
      <div className="login-card">
        ...
        <p className="sub-text">Authenticate to your secure network terminal.</p>

        <form onSubmit={handleLogin}>
          <div className="input-group">
            <label htmlFor="email">Email Address</label>
            <input
              id="email" type="email" placeholder="user@neoncity.com"
          ...
          <button type="submit" className={`submit-button ${loading ? "loading" : ""}`} disabled={loading}>
            <span className="button-content">
              {loading ? <><FiLoader className="spinner" /> AUTHENTICATING...</> : <><FiLogIn /> Sign In</>}
            </span>
          </button>
        </form>

        <div className="divider">OR ACCESS VIA NEON-NET</div>
        ...
        <p className="footer-text">
          No account found? <a href="/signup" className="signup-link">Initiate new user protocol</a>
        </p>
      ...
      <style jsx>{`
        /* ... Keep all your original root and container styles ... */
        :root {
          --dark-bg: #0d0d1a;
          --card-bg: rgba(20, 20, 40, 0.9);
          --neon-blue: #a8edea;
        ...
        .login-card { position: relative; z-index: 10; background: var(--card-bg); backdrop-filter: blur(25px); border-radius: 20px; border: 1px solid rgba(255, 255, 255, 0.1); padding: 50px 40px; width: 420px; color: white; text-align: center; }
        ...
        .submit-button { 
          width: 100%; padding: 16px; background: var(--button-gradient); color: white; border: none; border-radius: 12px; font-weight: 800; cursor: pointer; transition: all 0.4s; position: relative; overflow: hidden; 
        }

        /* --- NEW STYLES FOR GOOGLE --- */
        .divider {
          margin: 25px 0;
          display: flex;
          align-items: center;
